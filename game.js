  ...
  // Camera in grid units (not pixels)
  let camX = (N - 1) / 2;
  let camY = (N - 1) / 2;
  ...
  function screenToWorld(sx, sy) {
    const wx = (sx - viewW / 2) / cell + camX;
    const wy = (sy - viewH / 2) / cell + camY;
    return { x: wx, y: wy };
  }

  function worldToScreen(wx, wy) {
    return {
      x: (wx - camX) * cell + viewW / 2,
      y: (wy - camY) * cell + viewH / 2,
    };
  }
  ...
  function worldToNearestIntersection(wx, wy) {
    ...
    return { gx, gy, x: wrap(gx), y: wrap(gy) };
  }
  ...
  function draw() {
    ...
    const left = camX - (viewW / 2 + margin) / cell;
    const right = camX + (viewW / 2 + margin) / cell;
    const top = camY - (viewH / 2 + margin) / cell;
    const bottom = camY + (viewH / 2 + margin) / cell;
  ...
  function drawGrid(left, right, top, bottom) {
    ...
    // Horizontal lines (single stroke)
    ctx.beginPath();
  ...
  function onPointerMove(e) {
    ...
    mouse.over = { gx: near.gx, gy: near.gy, x: near.x, y: near.y };
    ...
    // Keep camera in a nice range to avoid float growth; modulo by N
    camX = ((camX % N) + N) % N;
    camY = ((camY % N) + N) % N;
  }
